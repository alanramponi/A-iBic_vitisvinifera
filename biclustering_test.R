...
min_value <- min(data_test_nan, na.rm=TRUE)   # -12.747
max_value <- max(data_test_nan, na.rm=TRUE)   # 10.208
min_range <- -13.00
max_range <- 11.00
nan_count <- sum(is.na(data_test_nan))        # 12395653
...
data_test_nan[is.na(data_test_nan)] <- random_number
dataset_mat <- data.matrix(data_test_nan)
...
rows_bic2 <-indexes$Bicluster4$Rows
cols_bic2 <- indexes$Bicluster4$Cols
rows_bic3 <-indexes$Bicluster7$Rows
cols_bic3 <- indexes$Bicluster7$Cols
rows_bic4 <-indexes$Bicluster8$Rows
cols_bic4 <- indexes$Bicluster8$Cols
